...
- Inputs:
  - `page_type` - pick the asset family; determines which CDN paths are scanned.
    - `character` - if `{{Character}}` includes explicit `style_id` and it is `2` or higher, character asset downloads use CDN/canonical filenames with `_st<style_id>` appended after the variant suffix (for example `3040088000_01_st2.png`) to avoid overwriting default-style canonical files. Redirect naming remains unchanged. Character `profile` assets are uploaded as `npc_profile_<id><index>.png` with `<Name>_profile*.png` redirects and categories `[[Category:Character Images]]` then `[[Category:Profile Room Character Images]]`. This lighter mode excludes character `f_skin` and `s_skin`.
    ...
    - `skin` - also checks `npc/profile` Profile Room outfit images using skin-style `Npc profile <id><index>.png` canonicals, `<Skin>_(<Character>)_profile*.png` redirects, and categories `[[Category:Outfit Images]]` then `[[Category:Profile Room Outfit Images]]`.
    ...
    - `item` - searches every `{{Item}}` template, reads `id`, `asset_type`, and `name`, downloads `https://prd-game-a-granbluefantasy.akamaized.net/assets_en/img/sp/assets/item/<asset_type>/s/<id>.jpg` and `/m/<id>.jpg`, uploads them as `item_<asset_type>_s/m_<id>.jpg`, creates redirects `<Name>_square.jpg` / `<Name>_icon.jpg` when `name` is present, and tags files with `[[Category:Item Images]]`. If `id` or `asset_type` is blank, both downloads for that item are skipped and reported.
    ...
    - `skill_icons` - extracts ability icon parameters from either the Character template or Class template and uploads the corresponding icons from the CDN. Character parameters: `a1_icon`, `a2_icon`, `a3_icon`, `a4_icon`, `a1a_icon`, `a2a_icon`, `a3a_icon`, `a4a_icon`, `a1b_icon`, `a2b_icon`, `a3b_icon`, `a4b_icon`. Class parameters: `s1_icon`, `s1_master_icon`, `s1_icon2`, `s2_icon`, `s3_icon`, `ex1_icon`, `ex2_icon`, `ex3_icon`, `ex4_icon`, `ex5_icon`, `ex1a_icon`, `ex2a_icon`, `ex3a_icon`, `ex4a_icon`, `umex1_icon`, `umex2_icon`, `umex3_icon`, `umex1a_icon`, `umex2a_icon`, `umex3a_icon`, `sup1_icon`, `sup2_icon`, `sup3_icon`, `sup4_icon`, `master_sup1_icon`, `master_sup2_icon`. Supports comma-separated values (e.g., `Ability_m_2232_3.png,Ability_m_2233_3.png`) and class-style spaced filenames such as `Ability m 2149_1.png` and `Ability s 9 1.jpg`. Icons are uploaded with canonical names matching parameter values. If no icons are found in the resolved template parameters, upload is skipped.
...
- Output: progress mentions current variant, then the summary lists variants processed, uploads, duplicates, total URLs checked, and direct wiki links for canonical/redirect targets (`item_<type>_s/m_<id>`, `<Name>_square/icon`).
...
**/eventupload**
- Usage: `/eventupload event_id:<biography042> event_name:<Event Name> asset_type:<notice|start|guide|trailer_mp3|voice_banner|top|raid_thumb|raid_thumb_collab|raid_thumb_records> max_index:<optional>`
...
- Inputs:
  ...
  - `asset_type` - strict dropdown with:
    ...
    - `raid_thumb_collab`: loads `img/sp/quest/assets/<event_id>_maniac_1.png`, `img/sp/quest/assets/<event_id>_maniac_2.png`, `img/sp/quest/assets/<event_id>_ex_1.png`, `img/sp/quest/assets/<event_id>_ex_2.png`, `img/sp/quest/assets/<event_id>_vhard.png`, `img/sp/quest/assets/<event_id>_hell.png`, `img/sp/quest/assets/<event_id>_hell_1.png`, `img/sp/quest/assets/<event_id>_hell_2.png`, `img/sp/quest/assets/<event_id>_hell_3.png`, `img/sp/quest/assets/<event_id>_group_multi.png`, `img/sp/quest/assets/<event_id>_group_1.png`, `img/sp/quest/assets/<event_id>_vhard_multi.png`, `img/sp/quest/assets/<event_id>_ex_multi_1.png`, and `img/sp/quest/assets/<event_id>_ex_multi_2.png`
    - `raid_thumb_records`: loads `img/sp/quest/assets/free/<event_id>_vhard.png`, `img/sp/quest/assets/free/<event_id>_ex.png`, `img/sp/quest/assets/free/<event_id>_high.png`, `img/sp/quest/assets/free/<event_id>_hell100.png`, and `img/sp/quest/assets/free/<event_id>_hell150.png`
  - `max_index` - optional upper bound for index probing (defaults to 20 for `notice`, `start`, `guide`, and `voice_banner`; `top` and `trailer_mp3` use a single fixed file and default to 1; `raid_thumb` currently processes the fixed `vhard`, `vhard_1`, `vhard_2`, `ex`, `ex_1`, `ex_2`, `high`, `high_1`, `high_2`, `hell`, `free_proud`, `free_proud_1`, and `free_proud_2` files and defaults to 13; `raid_thumb_collab` currently processes the fixed `maniac_1`, `maniac_2`, `ex_1`, `ex_2`, `vhard`, `hell`, `hell_1`, `hell_2`, `hell_3`, `group_multi`, `group_1`, `vhard_multi`, `ex_multi_1`, and `ex_multi_2` files and defaults to 14; `raid_thumb_records` currently processes the fixed `vhard`, `ex`, `high`, `hell100`, and `hell150` files and defaults to 5; minimum 1).
- Checks & Limits: same role/cooldown/lock behavior as other uploaders. `notice`, `start`, and `voice_banner` stop on the first missing base index; `guide` probes the base suffix plus `_0` and `_1` for each base index, tries `.jpg` first and `.png` second, skips missing subindices, and stops on the first missing base index; `top` and `trailer_mp3` each check one fixed URL; `raid_thumb`, `raid_thumb_collab`, and `raid_thumb_records` check every configured fixed variant even if some URLs are missing.
- Output: summary reports how many banners were processed/uploaded/duplicated along with wiki links for each canonical + redirect pair:
  ...
  - `raid_thumb_collab`: `quest_assets_<event_id>_maniac_1.png` and `BattleRaid_<EventName>_Maniac.png`, `quest_assets_<event_id>_maniac_2.png` and `BattleRaid_<EventName>_Maniac2.png` (with extra redirect `BattleRaid_<EventName>_Maniac_2.png`), `quest_assets_<event_id>_ex_1.png` and `BattleRaid_<EventName>_Solo_Extreme.png`, `quest_assets_<event_id>_ex_2.png` and `BattleRaid_<EventName>_Solo_ExtremePlus.png`, `quest_assets_<event_id>_vhard.png` and `BattleRaid <EventName> Solo Very Hard.png`, `quest_assets_<event_id>_hell.png` and `BattleRaid <EventName> Nightmare.png`, `quest_assets_<event_id>_hell_1.png` and `BattleRaid <EventName> Nightmare60.png`, `quest_assets_<event_id>_hell_2.png` and `BattleRaid <EventName> Nightmare100.png`, `quest_assets_<event_id>_hell_3.png` and `BattleRaid <EventName> Nightmare120.png`, `quest_assets_<event_id>_group_multi.png` and `BattleRaid_<EventName>_Raid_Thumb.png`, `quest_assets_<event_id>_group_1.png` and `BattleRaid_<EventName>_Solo_Thumb.png`, `quest_assets_<event_id>_vhard_multi.png` and `BattleRaid <EventName> Raid Very Hard.png`, `quest_assets_<event_id>_ex_multi_1.png` and `BattleRaid <EventName> Raid Extreme.png`, plus `quest_assets_<event_id>_ex_multi_2.png` and `BattleRaid <EventName> Raid ExtremePlus.png`
  - `raid_thumb_records`: `quest_assets_free_<event_id>_vhard.png` and `BattleRaid <EventName> Very Hard.png`, `quest_assets_free_<event_id>_ex.png` and `BattleRaid <EventName> Extreme.png`, `quest_assets_free_<event_id>_high.png` and `BattleRaid <EventName> Impossible.png`, `quest_assets_free_<event_id>_hell100.png` and `BattleRaid <EventName> Nightmare 100.png`, plus `quest_assets_free_<event_id>_hell150.png` and `BattleRaid <EventName> Nightmare 150.png`
  ...
  - `trailer_mp3`, `voice_banner`, `top`, `raid_thumb`, `raid_thumb_collab`, and `raid_thumb_records` skip the copy box and just list links.
