...
- `/imgupload page_type:<type> page_name:<title>` — bulk-upload all images for a wiki page (types: character, character_fs_skin, weapon, summon, class, class_skin, skin, npc, story_location, profile_stickers, profile_backgrounds, profile_other_characters, profile_favorite_art, profile_trophies, profile_trinkets, profile_frames, profile_designs, artifact, item, manatura, shield, skill_icons, bullet, advyrnture_gear, advyrnture_pal).
  - `page_type:character` supports `{{Character|style_id=...}}` for explicit styles (`style_id >= 2`). Styled uploads use canonical CDN/wiki filenames with `_st<style_id>` after the variant index (for example `..._01_st2.png`) so they do not overwrite default-style canonicals. Redirect naming is unchanged. Character `profile` images use `npc_profile_<id><index>.png` canonicals, `<Name>_profile*.png` redirects, and categories `[[Category:Character Images]]` then `[[Category:Profile Room Character Images]]`. Character `f_skin` / `s_skin` uploads are excluded from this lighter mode.
  - `page_type:skin` also checks `npc/profile` Profile Room outfit images using skin-style `Npc profile <id><index>.png` canonicals, `<Skin>_(<Character>)_profile*.png` redirects, and categories `[[Category:Outfit Images]]` then `[[Category:Profile Room Outfit Images]]`.
  - `page_type:item` reads every `{{Item}}` template's `id`, `asset_type`, and `name`; downloads from `/assets/item/<asset_type>/s/<id>.jpg` and `/m/<id>.jpg`; uploads as `item_<asset_type>_s/m_<id>.jpg`; creates `<Name>_square.jpg` / `<Name>_icon.jpg` redirects when `name` is present; and tags files with `[[Category:Item Images]]`.
  ...
  - `page_type:story_location` (shown in Discord as `story location`) uploads the `island_l` location header from `{{MainQuestTabs}}` or `{{EventTabs}}` using `location_id`, creates a redirect from `header_image` when it differs, and keeps duplicate images on the lowest base location id.
  - `page_type:profile_stickers` (shown in Discord as `profile (stickers)`) uploads Profile Room sticker images, square thumbnails, and icons from `{{ProfileRoom/Sticker/Row}}` rows and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Sticker Images]]`.
  - `page_type:profile_backgrounds` (shown in Discord as `profile (backgrounds)`) uploads Profile Room background images, square thumbnails, and icons from `{{ProfileRoom/Background/Row}}` rows, creates EN redirects, and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Background Images]]`.
  - `page_type:profile_other_characters` (shown in Discord as `profile (other characters)`) uploads Profile Room other-character images, icons, and squares from `{{ProfileRoom/OtherCharacter/Row}}` rows, creates EN redirects, and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Other Character Images]]`.
  - `page_type:profile_favorite_art` (shown in Discord as `profile (favorite art)`) uploads Profile Room favorite-art images, square thumbnails, and icons from `{{ProfileRoom/FavoriteArt/Row}}` rows, creates EN redirects, and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Favorite Art Images]]`.
  - `page_type:profile_trophies` (shown in Discord as `profile (trophies)`) uploads Profile Room trophy images, square thumbnails, and icons from `{{ProfileRoom/RoomTrophy/Row}}` rows, creates EN redirects, and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Trophy Images]]`.
  - `page_type:profile_trinkets` (shown in Discord as `profile (trinkets)`) uploads Profile Room trinket images, square thumbnails, and icons from `{{ProfileRoom/Trinket/Row}}` rows, creates EN redirects, and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Trinket Images]]`.
  - `page_type:profile_frames` (shown in Discord as `profile (frames)`) uploads Profile Room color-specific frame images and icons plus shared square thumbnails from `{{ProfileRoom/Frame/Row}}` rows, creates color-disambiguated EN square/icon redirects, and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Frame Images]]`.
  - `page_type:profile_designs` (shown in Discord as `profile (designs)`) uploads Profile Room color-specific design images and icons plus shared square thumbnails from `{{ProfileRoom/Design/Row}}` rows, creates color-disambiguated EN square/icon redirects, and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Design Images]]`.
  - `page_type:skill_icons` now supports both `{{Character}}` and `{{Class}}` templates. For `{{Character}}`, it scans `a1_icon`, `a2_icon`, `a3_icon`, `a4_icon`, `a1a_icon`, `a2a_icon`, `a3a_icon`, `a4a_icon`, `a1b_icon`, `a2b_icon`, `a3b_icon`, `a4b_icon`. For `{{Class}}`, it scans `s1_icon`, `s1_master_icon`, `s1_icon2`, `s2_icon`, `s3_icon`, `ex1_icon`, `ex2_icon`, `ex3_icon`, `ex4_icon`, `ex5_icon`, `ex1a_icon`, `ex2a_icon`, `ex3a_icon`, `ex4a_icon`, `umex1_icon`, `umex2_icon`, `umex3_icon`, `umex1a_icon`, `umex2a_icon`, `umex3a_icon`, `sup1_icon`, `sup2_icon`, `sup3_icon`, `sup4_icon`, `master_sup1_icon`, `master_sup2_icon`. Class icon filenames may use underscore or spaced forms such as `Ability_m_1273_1.png`, `Ability m 2149_1.png`, and `Ability s 9 1.jpg`.
  - Profile Room uploads accept `/imgupload filter:<id>` to upload only the row whose `id` exactly matches the filter value; `filter:all` processes the full table.
  - Local CLI equivalent: `uv run python images.py profile <stickers|backgrounds|other_characters|favorite_art|trophies|trinkets|frames|designs> <page name> [--filter <id>]` or `uv run python images.py profile_designs <page name> [--filter <id>]`.
  - Temporary local icon-only helper: add `--icons-only` to a Profile Room CLI command, or run `uv run python images.py profile_icons stickers="Profile Room/Stickers" backgrounds="Profile Room/Backgrounds" ...` with `type=page` pairs.
  - Temporary local character profile-image helper: `uv run python images.py character_profile "<Character Page>"` for one page, or `uv run python images.py character_profiles "<Category>" --resume "<Resume Page>"` to iterate a character category while uploading only `npc/profile` assets.
...
- `/eventupload event_id:<event folder id> event_name:<display name> asset_type:<notice|start|guide|trailer_mp3|voice_banner|top|raid_thumb|raid_thumb_collab|raid_thumb_records> max_index:<optional>` — upload event banner assets and create redirects:
