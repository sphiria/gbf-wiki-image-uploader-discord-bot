...
- `/imgupload page_type:<type> page_name:<title>` — bulk-upload all images for a wiki page (types: character, character_fs_skin, weapon, summon, class, class_skin, skin, npc, profile_stickers, profile_backgrounds, artifact, item, manatura, shield, skill_icons, bullet, advyrnture_gear, advyrnture_pal).
  ...
  - `page_type:profile_backgrounds` (shown in Discord as `profile (backgrounds)`) uploads Profile Room background images and thumbnails from `{{ProfileRoom/Background/Row}}` rows and tags files with `[[Category:Profile Room Images]]` and `[[Category:Profile Room Background Images]]`.
