...
- `/promoupdate promo_type:<suptix> promo_id:<id> end_date:<YYYY-MM-DD> end_time:<HH:MM> link_target:<wiki page>` — update a supported non-draw MainPageDraw promo subtemplate. `promo_id` may be the bare id, `banner_<id>`, `banner_<id>.png`, or the full CDN URL. This command does not upload the file.
