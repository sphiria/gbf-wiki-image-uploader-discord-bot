...
- `/imgupload page_type:<type> page_name:<title>` — bulk-upload all images for a wiki page (types: character, weapon, summon, class, skin, skill_icons, npc, artifact, item, manatura, shield, bullet).
...
- `/eventupload event_id:<event folder id> event_name:<display name> image_type:<banner_start|banner_notice> event_run:<default|redux|redux2|side_story>` — upload event banner assets (start banners map to `banner_event_start_<index>.png`, notice banners map to `banner_event_notice_<index>.png`) and create `banner_<EventName>_<index>.png`/`banner_<EventName>_notice_<index>.png` redirects.
- `/synccommands` — admin-only utility to force a guild/global slash-command sync if Discord stops showing new commands.

## Supported types

- Page types (`main.py:PAGE_TYPES`): `character`, `weapon`, `summon`, `class`, `skin`, `skill_icons`, `npc`, `artifact`, `item`, `manatura`, `shield`, `bullet`.
- Item types (`main.py:ITEM_TYPES`): `article`, `normal`, `recycling`, `skillplus`, `evolution`, `lottery`, `npcaugment`, `set`, `ticket`, `campaign`, `npcarousal`, `memorial`.
